...
export async function loginAction(
  ...
  // Redirect to original destination or role-appropriate default
  const dest = redirectTo && redirectTo.startsWith('/') ? redirectTo : getDefaultRoute(role, isAdmin);
  redirect(dest);
}

function getDefaultRoute(role: string, isAdmin: boolean): string {
